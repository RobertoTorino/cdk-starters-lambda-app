...
export class LambdaChecks implements IAspect {
    visit(node: IConstruct): void {
        if (node instanceof
            ...
            const resource = node.logicalId
            console.log(`Checking Lambda function: ${resource}`);
            ...
            if (!node.tracingConfig ||
                (!Tokenization.isResolvable(node.tracingConfig) &&
                    node.tracingConfig.mode !==
                    'Active')) {
                Annotations.of(node).
                addWarning(`\n` +
                    `🚫For Lambda function ${resource}\n` +
            ...
            Annotations.of(node).
            addInfo(`\n` +
                `✅ We found Lambda function ${resource}.\n`);
            ...
            // Check for Runtime
            if (node.runtime ||
                (!Tokenization.isResolvable(node.runtime) &&
                    node.runtime)) {
                Annotations.of(node).
                addInfo(`\n` +
                    `✅ For Lambda function ${resource}\n` +
            ...
            if (node.memorySize ||
                (!Tokenization.isResolvable(node.memorySize) &&
                    node.memorySize)) {
                Annotations.of(node).
                addInfo(`\n` +
                    `✅ For Lambda function ${resource}\n` +
